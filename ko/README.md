...
**업데이트:** Claude Code에 [내장 voice mode](https://x.com/bcherny/status/2032238378389840018)가 추가되었습니다. 테스트해보니 잘 작동하지만, 저는 개인적으로 여전히 로컬 모델을 사용하는데 더 빠르기 때문입니다.
...
YouTube 비디오에 대해 질문하거나 요약하고 싶다면 YouTube 비디오에서 "대본 보기"를 클릭한 다음 Cmd+A 또는 Ctrl+A를 하면 됩니다.
...
핵심은 올바른 정보 조각과 해당 정보 조각에 액세스하는 방법에 대한 지침을 제공하는 것입니다. `gh` 터미널 명령 액세스, 컨테이너 접근 방식(Tip 21), Gemini CLI를 통한 Reddit(Tip 11), Slack MCP와 같은 MCP를 통한 개인 정보, Cmd+A / Ctrl+A 방법(Tip 10)일 수 있습니다 - 무엇이든 상관없습니다. 또한 Claude Code가 특정 URL을 로드하는 데 문제가 있는 경우 Playwright MCP 또는 Claude의 기본 브라우저 통합을 시도할 수 있습니다(Tip 9 참조). 과학 연구를 위해 [paper-search](https://github.com/ykdojo/paper-search) plugin을 만들어 학술 논문을 검색할 수 있습니다.
